<template>
  <section class="file-list d-flex flex-column flex-grow-1">
    <div class="d-flex flex-row flex-grow-1 flex-wrap justify-center justify-md-start align-content-start"
      ...
      <div v-for="(workItem, index) in maps" :key="index" class="animated fadeIn">
        <slot name="card" v-bind="{ model: workItem, index: index }">
          <component :is="workItem.isFolder ? 'folder-item' : 'document-item'" :model="workItem"
            @check="check(workItem)" @open="open(workItem.objectId, index)" @rename="rename(workItem, index)"
            @remove="remove(workItem, index)" @share="share(workItem.objectId)"
            @remove_forever="removeForever(workItem, index)" @recovery="recovery(workItem)" @move_to="moveTo(workItem)"
            @copy_to="copyTo(workItem)" @copy="copyTo(workItem, true)"
...
import { last, pull, isEmpty, map, first } from "lodash";
