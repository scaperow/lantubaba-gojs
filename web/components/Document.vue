<template>
  <v-hover>
    <template v-slot:default="{ hover }">
      <v-card @click="$emit('open')" outlined width="182" class="ma-2 animated" :color="hover ? 'gray' : ''">
          ...
          <template v-slot:badge v-if="model.isShare === true">
            <v-tooltip bottom>
              <template v-slot:activator="{ on: tip }">
                <v-icon color="white" v-on="tip" @click.stop="$emit('share')"
        ...
        <v-img v-if="model.isDelete === true" class="document grey lighten-4" :src="model | thumbnail" :alt="model.name"
          fit="contain" :width="180" :height="180">
          <v-fade-transition>
            <!-- <v-menu>
        ...
        <v-img v-else class="document grey lighten-4 animated" v-ripple :src="cover" :alt="model.name" fit="contain"
          :width="180" :height="180" @click.prevent="$emit('open')"></v-img>

        <v-card-actions class="d-flex justify-space-between align-center">
          <label class="flex-grow-1 text-subtitle-2" :title="model.name" style="
          ...
          <div v-if="!model.isDelete">
            <!-- <v-tooltip bottom>
                <template v-slot:activator="{ on }">
                  <v-btn
                    icon
                    v-on="on"
                    @click.stop="$emit('change_accessibility')"
                  >
                    <v-icon :size="16">{{
            ...
            <v-btn @click.stop="$emit('check')" icon depressed style="position: absolute; top: 6px; left: 6px;" small
              v-show="hover || model.checked">
              <v-icon size="24" :color="model.checked ? 'primary' : ''">mdi-check-circle</v-icon>
            </v-btn>
            <v-menu offset-y v-if="model.isDelete !== true">
              <template v-slot:activator="{ on, value }">
                <v-btn style="position: absolute; top: 6px; right: 6px;" depressed small v-on="on"
                  v-show="hover || value">
